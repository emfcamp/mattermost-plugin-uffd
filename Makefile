...
ifneq ($(HAS_WEBAPP),)
	cd webapp && $(NPM) install --ignore-scripts && $(NPM) rebuild
	touch $@
endif
...
ifneq ($(HAS_SERVER),)
	...
	mockgen -destination=server/command/mocks/mock_commands.go -package=mocks github.com/lukegb/mattermost-plugin-uffd/server/command Command
endif
